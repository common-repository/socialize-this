...
class Twitter {
	...
	public $headerInfo, $oauth_token_secret, $oauth_token, $results, $screen_name, $oauthLink;
	...
	public function getOAuthLink(){
		return "http://api.twitter.com/oauth/authorize?".$this->oauthLink;
	}
	
	public function updateStatus($status, $params=''){
		$params['status'] = htmlentities($status);
		return $this->apiCall('http://api.twitter.com/1.1/statuses/update.json', $params);
	}
	public function mentions($params=''){
		return $this->apiCall('http://api.twitter.com/1.1/statuses/mentions_timeline.json', $params, NULL, 'GET');
	}
	public function sendDM($text, $screen_name, $params=''){
		$params['text'] = $text;
		$params['screen_name'] = $screen_name;
		return $this->apiCall('http://api.twitter.com/1.1/direct_messages/new.json', $params);
	}
	
	public function addUserToList($list_id, $user, $id){
		$params['id'] = htmlentities($id);
		return $this->apiCall('http://api.twitter.com/1.1/'.$user.'/'.$list_id.'/members.json', $params);
	}
	...
	public function getList($list_id, $user, $since_id='0'){
		...
		$params['per_page'] = '200';
		return $this->apiCall('http://api.twitter.com/1.1/'.$user.'/lists/'.$list_id.'/statuses.xml', $params, NULL, 'GET');
	}
	
	public function getTimeline($screen_name,$count=100){
		$params['screen_name'] = $screen_name;
		$params['count'] = $count;
		return $this->apiCall('http://api.twitter.com/1.1/statuses/user_timeline.json', $params, NULL, 'GET');
	}
